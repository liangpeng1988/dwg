...
export class LibreDwgConverter {
  ...
  convert(data: Dwg_Data_Ptr) {
    ...
    // 构建blocks映射，从 BLOCK_RECORD 表中提取块定义
    db.blocks = new Map()
    for (const blockRecord of db.tables.BLOCK_RECORD.entries) {
      if (blockRecord.name && blockRecord.entities) {
        db.blocks.set(blockRecord.name, blockRecord.entities)
      }
    }
    
    console.log(`[LibreDwgConverter] Built ${db.blocks.size} block definitions from BLOCK_RECORD table`)
